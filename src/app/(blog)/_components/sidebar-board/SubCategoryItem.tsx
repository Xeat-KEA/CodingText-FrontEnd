...
const SubCategoryItem: React.FC<SubCategoryItemProps> = ({
  ...
  const [hoveredSubCategoryId, setHoveredSubCategoryId] =
      ...
      {editSubCategoryId[category.id] === subCategory.id ? (
        ...
        <>
          <Link href={`/blog/${blogId}/${category.id}/${subCategory.id}`}>
            <p
              className={
                (Number(params.categoryId) == category.id &&
                  Number(params.subCategoryId) == subCategory.id) ||
                (params.postId &&
