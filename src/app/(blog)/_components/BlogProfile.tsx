...
import { Blog_Profile_Data, REPORT_REASONS } from "../_constants/constants";
...
export default function BlogProfile() {
  ...
  const [selectedOption, setSelectedOption] = useState<Selection | null>(null); // 신고 사유 선택
  ...
  return (
    <>
      <div className="w-226 h-30 mt-12 mb-6 p-2">
        ...
        {Blog_Profile_Data.map((profile, index) => {
          ...
          const handleFollowerClick = () => {
            setFollowerCount(
              (prevCount) => prevCount + (isFollowerClicked ? -1 : 1)
            );
          ...
          return (
            <div
              key={index}
              className="profile-card flex items-center space-x-6"
            >
              {/* 프로필 이미지 */}
      ...
      {isReportDialogOpen && (
        ...
        >
          <DropDown
            selection={selectedOption?.content || ""}
            list={REPORT_REASONS}
            onSelectionClick={setSelectedOption}
            placeholder="분류"
          />
          {selectedOption?.content === "직접 입력" && (
            <div>
              <textarea
                value={customInput}
