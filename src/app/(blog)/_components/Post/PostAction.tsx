...
import { BpEditIcon, BpFollowerIcon, BpReportIcon, ShareIcon, SmDeleteIcon } from "../Icons";
...
const PostAction: React.FC<PostProps> = ({
    currentPost,
}) => {
    // 전역 변수
    const {
    ...
    const confirmDeletePost = () => {
        ...
        router.push(`/blog/${params?.id}/${currentPost?.categoryId}/${currentPost?.subCategoryId}`)
    }
            ...
            <div className="flex gap-4">
                ...
                {isOwnBlog ? (
                    ...
                    {/* 수정을 위해 필요한 ? currentPost 넘겨주기..?  경로 수정 필요*/}
                        ...
                        <button className="flex items-center gap-1" onClick={() => onClickDeletePost(Number(currentPost?.postId))}>
                            <SmDeleteIcon />
                            <p className="text-red text-xs font-semibold">{`삭제`}</p>
                        </button>
                    </>
                ) : (
                    <button className="flex items-center gap-1" onClick={() => onClickReportPost(Number(currentPost?.postId))}>
                        <BpReportIcon />
