...
const Comment: React.FC<CommentProps> = ({
    ...
    // 댓글 작성자의 프로필
    const userProfile = Blog_Profile_Data.find((profile) => profile.profileId === userId);

    // 언급된 사용자의 프로필
    const mentionProfile = Blog_Profile_Data.find((profile) => profile.profileId === mentionId);

    return (
        <div className={`flex flex-col w-full gap-4 py-4 border-b border-border-2 ${mentionId ? 'ml-12' : ''}`}>
            <div className="flex w-full justify-between items-center">
                <div className="flex gap-2 items-center">
                    ...
                    <p className="text-xs text-body font-semibold">{userProfile?.name}</p>
                </div>
                <p className="text-xs text-body font-body">{useCalculateDate(createdAt)}</p>
            </div>
            ...
            <div className="flex w-full h-5 justify-between items-center">
                <button className="flex items-center gap-1" onClick={() => onReplyClick(replyId, userId)}>
                    <ReplyIcon />
                    <p className="text-black text-xs font-semibold ">답글</p>
                </button>
                {isOwnComment ? (
                    <button className="flex items-center gap-1" onClick={() => onDelete(replyId)}>
                        <SmDeleteIcon />
                        <p className="text-red text-xs font-semibold ">삭제</p>
                    </button>
                ) : (
                    <button className="flex items-center gap-1" onClick={() => onReport(replyId)}>
                        <BpReportIcon />
